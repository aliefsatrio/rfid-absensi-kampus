    ...
    public SlidingStatusToggle() {
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                // PERBAIKAN: Menggunakan pemicu via kelas Preferences langsung atau diserahkan ke Form utama
                if (clickX < sectionWidth) {
                    setSelectedLanguageIndex(0);
                    try { java.util.prefs.Preferences.userRoot().node("RFIDAbsensi").put("LANGUAGE", langcodes[0]); } catch(Exception ex){}
                } else if (clickX < sectionWidth * 2) {
                    setSelectedLanguageIndex(1);
                    try { java.util.prefs.Preferences.userRoot().node("RFIDAbsensi").put("LANGUAGE", langcodes[1]); } catch(Exception ex){}
                } else {
                    setSelectedLanguageIndex(2);
                    try { java.util.prefs.Preferences.userRoot().node("RFIDAbsensi").put("LANGUAGE", langcodes[2]); } catch(Exception ex){}
                }
